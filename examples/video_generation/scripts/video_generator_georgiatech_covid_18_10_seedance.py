...
DEFAULT_ERROR_INSTRUCTIONS_JSON = str(
    REPO_ROOT / "data" / "examples" / "split_50_error_instructions_openai_judged.json"
)
...
def concat_with_crossfade(
    clips: List[str], out_mp4: str, fade: float = 0.12, fps: float = 30.0
) -> None:
    assert len(clips) >= 1
    Path(out_mp4).parent.mkdir(parents=True, exist_ok=True)
    ...
    for i in range(1, len(clips)):
        ...
        fc.append(
            f"{v_prev}{v_in}xfade=transition=fade:duration={fade:.6f}:offset={off:.6f}[vxf{i}]"
        )
...
def load_error_rewrite_for_take(error_json_path: str, take_name: str) -> Optional[dict]:
    ...
    for _uid, info in takes.items():
        if isinstance(info, dict) and info.get("take_name") == take_name:
            return info
    print(
        f"WARNING: take_name='{take_name}' not found in error instructions json: {error_json_path}"
    )
    return None
...
def parse_args():
    ...
    # splice params
    p.add_argument(
        "--fade", type=float, default=0.12, help="Crossfade seconds at splice boundaries."
    )
    ...
    p.add_argument(
        ...
        type=float,
        default=274.0,
        help="Absolute replacement window start (seconds).",
    )
    p.add_argument(
        "--replace_to", type=float, default=287.0, help="Absolute replacement window end (seconds)."
    )
    ...
    p.add_argument(
        "--base_url", default="", help="Override base URL (else env ARK_BASE_URL or default)."
    )
    ...
    p.add_argument(
        "--duration", type=int, default=0, help="0 => auto from window, clamped to [2,12]."
    )
    p.add_argument(
        "--generate_audio", action="store_true", help="Ask Seedance to generate audio too."
    )
...
if __name__ == "__main__":
    ...
    if args.replace_from is not None and args.replace_to is not None:
        ...
        print(
            f"Replace window (absolute): {splice_from:.3f}–{splice_to:.3f} (len={splice_to - splice_from:.3f}s)"
        )
    else:
        ...
        print(
            f"Replace window (step): {splice_from:.3f}–{splice_to:.3f} (len={splice_to - splice_from:.3f}s)"
        )
    ...
    print(
        f"Generated duration: {gen_dur:.3f}s | removed window: {removed_len:.3f}s | delta: {gen_dur - removed_len:+.3f}s"
    )
    ...
    if has_audio:
        ...
        if has_part2 and part2_raw is not None:
            ...
            extract_audio_segment(
                audio_source,
                float(splice_to) + aoff,
                float(specs["duration"] - splice_to),
                str(p2a),
            )
